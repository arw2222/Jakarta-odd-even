...
#-------------------------------------------------------------------------------
## Satelite pollution observation
#-------------------------------------------------------------------------------
...
#boxplot
ggplot(event_data, aes(x = as.factor(tot_month_dif), y = pm2.5, fill = od)) +
  geom_boxplot(alpha = 0.5) +
  geom_vline(xintercept = as.factor(-1), linetype = "dashed", color = "red") +  # Convert to numeric
  labs(
    x = "Months Since Odd-Even Policy",
    y = "Pollution PM 2.5 (microgram/m3)"
  ) +
...
#Event Study with rainfall (robustness check)
fe2 <- feols(
